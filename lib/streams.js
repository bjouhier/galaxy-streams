...
/// see the [galaxy tutorial](https://github.com/bjouhier/galaxy/blob/master/tutorial/tutorial.md)
/// 
...
function Wrapper(emitter) {
	...
	self.close = galaxy.star(function(callback) {
...
function ReadableStream(emitter, options) {
	...
	var readChunk = galaxy.star(function(callback) {
		if (_chunks.length > 0) {
	...
	self.read = function *(len) {
		...
		if (len == null) return yield readChunk();
		if (len < 0) len = Infinity;
		if (len == 0) return _encoding ? "" : new Buffer(0);
		var chunks = [],
			total = 0;
		while (total < len) {
			var chunk = yield readChunk();
...
function WritableStream(emitter, options) {
	...
	var _drain = galaxy.star(function(callback) {
	...
	self.write = function *(data, enc) {
		...
		if (data != null) {
			...
			//
			if (!emitter.write(data, enc)) yield _drain();
		} else {
			emitter.end();
		}
		return self;
...
function Server(emitter) {
	...
	self.listen = function *(args) {
		...
		return yield galaxy.star(function(callback) {
...
function HttpServer(requestListener, options) {
	...
	var emitter = options.createServer(function(request, response) {
		galaxy.context = {};
		return galaxy.unstar(requestListener)(new HttpServerRequest(request, options), new HttpServerResponse(response, options), function(err) {
			if (err) {
	...
	if (options.proxy) {
		...
		if (options.proxy.auth) {
			if (options.proxy.auth.toLowerCase() === "basic") {
				...
				//options.headers["Proxy-Authorization"] = "NTLM " + yield ntlm.authenticate(options.proxy);	
...
function HttpClientRequest(options) {
	...
	self.response = galaxy.star(function(callback) {
...
function NetClient(options, args) {
	...
	self.connect = galaxy.star(function(callback) {
...
function NetServer(serverOptions, connectionListener, streamOptions) {
	...
	var emitter = net.createServer(serverOptions, function(connection) {
		galaxy.context = {};
		galaxy.spin(connectionListener(new NetStream(connection, streamOptions || {})));
	});
	Server.call(self, emitter);
}
...
exports.usingReadable = function *(emitter, options, fn) {
	return yield exports.using.call(this, exports.ReadableStream, emitter, options, fn);
}
...
exports.usingWritable = function *(emitter, options, fn) {
	return yield exports.using.call(this, exports.WritableStream, emitter, options, fn);
}

/// * `yield streams.pump(inStream, outStream)`  
///    Pumps from `inStream` to `outStream`.  
///    Does not close the streams at the end.
exports.pump = function *(inStream, outStream) {
	var data;
	while (data = yield inStream.read()) yield outStream.write(data);
}
/// 
/// ## Encoding detection
/// 
